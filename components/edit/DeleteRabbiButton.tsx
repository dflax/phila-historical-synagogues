...
export default function DeleteRabbiButton({ profile, affiliationCount, photoCount }: Props) {
  ...
  // Count non-null biographical fields
  const bioFieldCount = [
    profile.biography,
    profile.birth_year,
    profile.death_year,
    ...
    profile.languages?.length ? true : null,
  ...
  async function handleSubmit() {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        synagogue_id:   null,
        proposal_type:  'rabbi_profile_delete',
        proposed_data: {
          action:              'delete',
          biographical_fields: bioFieldCount,
  ...
  // ── Bullet row in the summary step ───────────────────────────────────────────
    ...
    <>
      ...
      {step !== 'closed' && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Propose rabbi deletion"
        >
          <div
            ...
            <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-700">
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                  {submitted ? 'Deletion Proposed' : 'Delete Rabbi'}
                </h2>
              ...
              {!submitted && step === 'summary' && (
                <div className="space-y-5">
                  ...
                  <ul className="space-y-3">
                    <SummaryRow count={1} label={`Profile page for ${profile.canonical_name}`} />
                    <SummaryRow
                      count={bioFieldCount}
                      label={`Biographical ${bioFieldCount === 1 ? 'field' : 'fields'} (biography, birth/death years, seminary, denomination, etc.)`}
                    />
                    <SummaryRow
                      count={affiliationCount}
                      label={`Synagogue affiliation ${affiliationCount === 1 ? 'record' : 'records'}${affiliationCount === 0 ? ' (none on file)' : ''}`}
                    />
                    <SummaryRow
                      count={photoCount}
                ...
                <div className="space-y-4">
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-4 py-3">
                    ...
                    <p className="text-sm text-red-600 dark:text-red-400">
                      You are proposing to delete <strong>{profile.canonical_name}</strong> and all {plural(affiliationCount, 'affiliation')} and {plural(photoCount, 'photo')} on record.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Why should this entry be removed? (e.g. 'Duplicate of another profile', 'Created in error', 'Fictional person')"
                    />
                  </div>
