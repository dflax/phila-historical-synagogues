...
import CreateRabbiForm from './CreateRabbiForm'

export default function CreateRabbiButton() {
    ...
    <>
      ...
      {formOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Add a new clergy member profile"
        >
          <div
          ...
          >
            ...
            <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">Add Clergy Member</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Propose a new profile for editorial review
                </p>
              </div>
              <button
                onClick={closeModal}
              ...
              {submitted ? (
                ...
                <div className="text-center py-6 space-y-3">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Your proposal is pending review. Once approved by an editor, the profile will appear in the directory.
                  </p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">This window will close automatically…</p>
                </div>
              ) : (
                // ── Form ─────────────────────────────────────────────────────
                <CreateRabbiForm userId={user!.id} onSuccess={handleSuccess} />
              )}
            </div>
          </div>
        </div>
