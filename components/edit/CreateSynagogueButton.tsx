...
import CreateSynagogueForm from './CreateSynagogueForm'

export default function CreateSynagogueButton() {
  ...
  // Don't render until auth state is known (prevents flash)
  if (!authReady) return null

  const buttonClass =
    'flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition shadow-sm'
    ...
    <>
      ...
      {formOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Add a new synagogue"
        >
          <div
          ...
          >
            ...
            <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-700">
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">Add a Synagogue</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Propose a new entry for editorial review
                </p>
              </div>
              <button
                onClick={closeModal}
              ...
              {submitted ? (
                ...
                <div className="text-center py-6 space-y-3">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Your proposal is pending review. Once approved by an editor, the synagogue will appear in the directory.
                  </p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">This window will close automatically…</p>
                </div>
              ) : (
                // ── Form ─────────────────────────────────────────────────────
                <CreateSynagogueForm userId={user!.id} onSuccess={handleSuccess} />
              )}
            </div>
          </div>
        </div>
