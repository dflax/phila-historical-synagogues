...
- **Photo thumbnail in Admin Dashboard** — `ProposalCard` in `AdminClient.tsx` now renders a photo summary block for `image_upload` proposals: thumbnail preview, caption, entity name, photographer, dimensions, and file size; `IMAGE_UPLOAD_HIDE_FIELDS` prevents the raw metadata from also appearing in the generic diff table; `storageBaseUrl` prop flows from `admin/page.tsx` → `AdminClient` → `ProposalCard`
